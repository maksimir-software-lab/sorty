...
final class ItemTypeClassifier {
	...
	static SortKey classify(String itemPath, boolean blockItem, boolean foodItem) {
		...
		if (itemPath.endsWith("_dust") || itemPath.endsWith("_powder")
				|| itemPath.equals("redstone") || itemPath.equals("gunpowder")) {
		...
		return miscellaneousType != null
				? miscellaneousType
				: key(MISCELLANEOUS, 99, "miscellaneous", itemPath, 0);
	}
	...
	private static SortKey classifyEquipment(String path) {
		...
		if (path.equals("shears") || path.equals("brush")
		...
		if (path.equals("bow") || path.equals("crossbow")
				|| path.equals("trident") || path.equals("mace") || path.endsWith("_spear")) {
	...
	private static boolean isFunctionalBlock(String path) {
		return FUNCTIONAL_BLOCK_NAMES.contains(path)
	...
	private static SortKey suffixKey(
			int category,
			int typeOrder,
			String type,
			String path,
