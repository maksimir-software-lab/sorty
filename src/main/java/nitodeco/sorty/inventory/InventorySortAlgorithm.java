...
public final class InventorySortAlgorithm {
	private static final StackOperations<ItemStack> ITEM_STACK_OPERATIONS =
			new StackOperations<>() {
				@Override
				...
				public ItemTypeClassifier.SortKey sortKey(ItemStack stack) {
					...
					return ItemTypeClassifier.classify(
							path,
	...
	static <T> List<T> sort(List<T> slots, StackOperations<T> operations) {
		...
		Comparator<T> order = Comparator
				.comparing(operations::sortKey)
				.thenComparing(operations::tieBreakKey);
		nonEmpty.sort(order);
	...
	private static <T> List<T> mergeCompatibleStacks(
			List<T> sorted,
		...
		for (T source : sorted) {
			...
			for (int targetIndex = 0; targetIndex < result.size(); targetIndex++) {
				...
				if (moved > 0) {
					result.set(
							targetIndex,
							operations.copyWithCount(target, operations.count(target) + moved)
					);
					remaining -= moved;
				}
				if (remaining == 0) {
